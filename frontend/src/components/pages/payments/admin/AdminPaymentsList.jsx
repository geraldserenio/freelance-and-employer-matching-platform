...
import { Confirm } from "../business/Confirm";
...
export const AdminPaymentsList = () => {
  ...
  useEffect(() => {
    async function fetchPayments() {
      // if (loggedInUser?.user?.user_type !== "admin") {
      //   Swal.fire({
      ...
      //   });
      // }
      const list = await getAllPayments(status, page, loggedInUser?.token);
      if (list?.status === 401) {
        Swal.fire({
  ...
  const handleDeleteJob = (row) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You are about to return a fund to a client.",
      icon: "warning",
      confirmButtonText: "OK",
      allowOutsideClick: false,
      allowEscapeKey: false,
