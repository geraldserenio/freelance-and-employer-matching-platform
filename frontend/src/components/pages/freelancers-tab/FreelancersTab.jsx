...
export const FreelancersTab = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const howItWorksResult = await getHowItWorks("freelancers");
        const testimonialsResult =
          await getTestimonialsByUserType("freelancers");
        const whyChooseLiberResult =
          await getWhyChooseLiberByUserType("freelancers");
    ...
    <div>
      {loading ? (
        ...
        <div>
          <PageHeader />
          <HeadingSection />
          <SignUpAsSection btnText={"Sign up as a freelancer"} />
          <HowItWorks howItWorks={howItWorks} />
          <WhyChooseLiber userType={"freelancer"} perks={perks} />
          <Reviews
            heading={"What freelancers say:"}
